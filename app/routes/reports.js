...
export default Ember.Route.extend({
        ...
        if(data.success === true) {
          ...
          data.data.forEach(function(item) {
            ...
            for(var x = 0; x < item.bingers.length; x++) {
              ...
              //tempBingers.addObject(item);

              var binger = self.store.push('binger', item.bingers[x]);

              bingerArray.addObject(binger);

              console.log('[ReportsRoute.model] binger id: ', JSON.stringify(item.bingers[x], null, 2));
            }
  ...
  actions: {
    error: function(reason) {
      console.log('[ConsoleRoute.error] error: ', JSON.stringify(reason)); // "FAIL"
